...
# 提取数据
data=pd.read_hdf('../data/result_generalsaving.h5')
...
portfolios = ['very_conservative_av_50','conservative_av_50','balanced_av_50','aggressive_av_50','very_aggressive_av_50']
...
# 以下参数为该页面用户输入的
type = int(sys.argv[1])
purchase = int(sys.argv[2])#100000
pur_time = int(sys.argv[3])#8
income = int(sys.argv[4])#5000
initial = int(sys.argv[5])#0
topup = int(sys.argv[6])#700
rec_topup = int(sys.argv[7])


## 此时rec_topup为用户拖动选择的值
...
rec_topup_1 = pv / float((((1 + data[portfolios[m]].iloc[pur_time * 12 + 1]) ** (pur_time * 12 + 1) - 1) /
                       data[portfolios[m]].iloc[pur_time * 12 + 1]) - 1)
slidermax = int(rec_topup_1) + 1


# 计算rec_initial
pv = purchase*(1.02**pur_time)
fv = rec_topup * ((((1 + data[portfolios[m]].iloc[pur_time * 12 + 1]) ** (pur_time * 12 + 1) - 1) /
                       float(data[portfolios[m]].iloc[pur_time * 12 + 1]) - 1))
if fv > pv:
    rec_topup = pv / float((((1 + data[portfolios[m]].iloc[pur_time * 12 + 1]) ** (pur_time * 12 + 1) - 1) /
                                 data[portfolios[m]].iloc[pur_time * 12 + 1]) - 1)
    rec_initial = 0
else:
    rec_initial = (pv - fv) / float((1 + data[portfolios[m]].iloc[pur_time * 12 + 1]) ** (pur_time*12))
